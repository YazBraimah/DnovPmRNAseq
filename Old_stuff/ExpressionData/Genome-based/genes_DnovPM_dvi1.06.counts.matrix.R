...
if (file.exists("genes_DnovPM_dvi1.06.counts.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("genes_DnovPM_dvi1.06.counts.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("genes_DnovPM_dvi1.06.counts.matrix", header=T, com='', sep="\t", row.names=1, check.names=F)
    primary_data = as.matrix(primary_data)
}
source("/Users/yazahmed/Programs/trinityrnaseq-2.1.1/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/Users/yazahmed/Programs/trinityrnaseq-2.1.1/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/Users/yazahmed/Programs/trinityrnaseq-2.1.1/Analysis/DifferentialExpression/R/pairs3.R")
data = primary_data
...
write.table(data, file="genes_DnovPM_dvi1.06.counts.matrix.minCol10.minRow10.log2.dat", quote=F, sep='	');
gene_cor = NULL
